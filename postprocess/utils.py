...
def object_prediction_list_to_torch(object_prediction_list: ObjectPredictionList) -> torch.tensor:
    ...
    if True:#object_prediction_list.points is not None:
        torch_predictions = torch.zeros([num_predictions, 19, 2], dtype=torch.float32)
        for ind, object_prediction in enumerate(object_prediction_list):
            torch_predictions[ind, :17] = torch.tensor(object_prediction.tolist().points.to_cxcy(), dtype=torch.float32)
            torch_predictions[ind, 17] = object_prediction.tolist().score.value[0]
            torch_predictions[ind, 18] = object_prediction.tolist().category.id
