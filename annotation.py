...
class PoseKeypoints:
    """
    PoseKeypoints of the annotation.
    """

    def __init__(self, points: np.ndarray, shift_amount: List[int] = [0, 0]):
        """
        Args:
            points: List[float]
                N*[cx, cy, visibility]
    ...
    def get_shifted_box(self):
        """
        Returns: shifted PoseKeypoints
        """
        self.points[:,0] = self.points[:,0] + self.shift_x
        self.points[:,1] = self.points[:,1] + self.shift_y
        
        return PoseKeypoints(self.points)
...
class ObjectAnnotation:
    ...
    def __init__(
        ...
        if ((bbox is None) or (bool_mask is None)) and (points is None):
            raise ValueError("you must provide a bbox/points or bool_mask")
        ...
        # if points is a numpy object, convert it to python List[float]
        # if type(points).__module__ == "numpy":
        #     points = copy.deepcopy(points).tolist()
        ...
        if points is not None:
            self.points = PoseKeypoints(points)
    ...
    def get_shifted_object_annotation(self):
        if self.mask:
            return ObjectAnnotation(
                ...
                return ObjectAnnotation(
                    points=self.points.get_shifted_box().to_xyxy(),
                    category_id=self.category.id,
                    bool_mask=None,
                    category_name=self.category.name,
                    shift_amount=[0, 0],
